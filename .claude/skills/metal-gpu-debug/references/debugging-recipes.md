...
**Expected TOC output structure:**
```xml
<trace-toc>
  <run number="1">
    <data>
      <table schema="metal-driver-event-intervals" .../>
      <table schema="gpu-counter-intervals" .../>
...
# 4. Parse for GPU work durations
python3 << 'EOF'
import xml.etree.ElementTree as ET

tree = ET.parse('gpu_events.xml')
root = tree.getroot()

# Build ID→value lookup for reference nodes
id_map = {}
...
def resolve(elem):
    """Resolve ref nodes to their original values."""
    ref = elem.get('ref')
...
for row in rows[:20]:  # First 20 events
    cols = list(row)
    vals = [resolve(c).get('fmt', resolve(c).text or '?') for c in cols]
    print('\t'.join(vals))
EOF
...
# 6. Open in Instruments for visual timeline
open perf.trace
```
...
# 4. Parse
python3 parse_trace.py ios_gpu_events.xml --summary
